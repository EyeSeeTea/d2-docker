...
def run(args):
    ...
    if os.path.exists(image_or_file):
...
def import_from_file(images_path):
    result = import_.load_images_file(images_path)
    ...
    if dhis2_db_images:
        ...
        raise D2DockerError("Cannot find dhis2-db image (pattern={})".format(DHIS2_DB_IMAGE_RE))
